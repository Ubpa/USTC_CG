...
void Attribute::ComponentVisitor::ImplVisit(Ptr<CmptSimulate> simulate) {
	...
	grid->AddButton("set x max fix", [simulate]() {
	...
	grid->AddButton("set Euler Method", [simulate]() {
		simulate->SetEulerMethod();
		});
	grid->AddButton("set PD Method", [simulate]() {
		simulate->SetPDMethod();
		});
}
...
void Attribute::ComponentVisitor::ImplVisit(Ptr<TriMesh> mesh) {
	...
	Ptr<Parameterize::Method> pWeightMethod = std::make_shared<Parameterize::Method>();
	*pWeightMethod = Parameterize::Method::kUniform;
	grid->AddComboBox("Weight Method", parameterizingWMethodStrs, [pWeightMethod](const std::string& str){
			if (str == "Uniform")
				*pWeightMethod = Parameterize::Method::kUniform;
			else if (str == "Cotangent")
				*pWeightMethod = Parameterize::Method::kCotangent;
		}
	);
	std::vector<std::string> parameterizingBMethodStrs = { "Circle", "Square" };
	Ptr<Parameterize::BoundaryType> pBoundaryType = std::make_shared<Parameterize::BoundaryType>();
	*pBoundaryType = Parameterize::BoundaryType::kCircle;
	grid->AddComboBox("Boundary Method", parameterizingBMethodStrs, [pBoundaryType](const std::string& str)
		{
			if (str == "Circle")
			*pBoundaryType = Parameterize::BoundaryType::kCircle;
			else if (str == "Square")
				*pBoundaryType = Parameterize::BoundaryType::kSquare;
		});

	grid->AddButton("Texture Image", [mesh, pOGLW = attr->pOGLW, pWeightMethod, pBoundaryType]() {
			auto Parameterize = Parameterize::New(mesh);
			Parameterize->SetMethod(*pWeightMethod);
			Parameterize->SetBoundary(*pBoundaryType);
			if (Parameterize->Run())
	...
	grid->AddButton("Parameterize Show", [mesh, pOGLW = attr->pOGLW, pWeightMethod, pBoundaryType]() {
			auto Parameterize = Parameterize::New(mesh);
			Parameterize->SetMethod(*pWeightMethod);
			Parameterize->SetBoundary(*pBoundaryType);
			Parameterize->SetDisplay();
