...
int main() {
  ...
  mpm m;
  // m.add_square(Vec(0.55, 0.45), 0x87CEFA, Water);
  // m.add_square(Vec(0.45, 0.65), 0xFFFAFA, Snow);
  // m.add_square(Vec(0.55, 0.85), 0xED553B, Jelly);
  ...
  m.add_obstacle(Vec(0.1, 0.1), 40);
  m.add_obstacle(Vec(0.9, 0.1), 40);
  m.add_obstacle(Vec(0.1, 0.9), 40);
  m.add_obstacle(Vec(0.9, 0.9), 40);
  int frame = 0;
  // Main Loop
  for (int step = 0;; step++) {
    // Advance simulation
    ...
    if (step % int(frame_dt / dt) == 0) {
      // Clear background
      canvas.clear(0x44803F);
      // Box
      canvas.rect(Vec(0.04), Vec(0.96)).radius(2).color(0xD0ECF2).close();
    ...
    for (int j = 0; j <= n; j++) {
      ...
      if (g[2] > 0) {
        ...
        for (size_t k = 0; k < obstacles.size(); k++){
            ...
            real radius = obstacles[k][2];
            Vec dist = Vec(i, j) * dx - center;
            real dist2 = sqrt(dist.x * dist.x + dist.y * dist.y);
            if (dist2 <= radius*dx / DX) {
                dist = dist / dist2;
