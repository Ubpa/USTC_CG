...
namespace Ubpa {
	...
	private:
		...
		std::vector<float> alias_prob_list;
		std::vector<size_t> alias_index_list;
		std::vector<float> env_prob_list;

		
		std::tuple<rgbf, vecf3, float> aliasSample(const normalf& n)const noexcept;
		
		void aliasInit(const std::vector<float>& prob_list);
		
		float aliasPDF(const vecf3& dir) const noexcept;
